# "Data Comes Alive: An Introduction to Data Visualization with Python" Workshop Materials 📊

This repository contains comprehensive workshop materials from [Hamid Namjoo](https://hamidnamjoo.com/)'s professional Data Visualization workshop, conducted in **Fall 2025** at [**Islamic Azad University of Shiraz**](https://shiraz.iau.ir/en). Transform raw data into compelling visual narratives using Python's most powerful visualization libraries.

---

> ## 📦 <strong>DVW.rar</strong> is All You Need

**🎯 Complete Workshop Package** - Download everything in one convenient bundle:
[📥 **Download Complete Workshop Materials (RAR)**](https://drive.google.com/file/d/1LIpfSKWoIat35S_npCTt5Q4gRRH5lAon/view?usp=sharing) 
*Contains: Slides (ODP, PPTX & PDF), Jupyter notebook(IPYNB, HTML & PDF), datasets, and supplementary resources*

---

## Need something specific? Browse individual files below 👇

## 🖼️ Presentation Slides

**📊 Google Slides** - [🔗 Interactive Presentation](https://docs.google.com/presentation/d/1vxnBW_orrz3A9QKGCsFrdp3aWZhsnVe9_hzLaKdRmcg/edit?usp=sharing)

**📄 PDF Version** - [🔗 Download PDF Slides](https://drive.google.com/file/d/1yB5z2LJMwg_Y6dAXa2X9EVA3DvSV9bjh/view?usp=sharing)

**💼 PowerPoint Version** - [🔗 Download PPTX Slides](https://docs.google.com/presentation/d/1UQb_tgbnmajPJsnRIzz2HqpLAJj_tKru/edit?usp=sharing&ouid=111546761383517093470&rtpof=true&sd=true)

**✅ OpenDocument Presentation** - [🔗 Download ODP Slides](https://drive.google.com/file/d/1Rt8EQyEzaGsgAKfbimy8C3VPgLsyen8J/view?usp=sharing) *(OpenSource✅ alternative to PPTX --> So, first download it then open it with [Impress by LibreOffice](https://www.libreoffice.org/discover/impress/))*

---

## 📓 Jupyter Notebook

**🚀 Google Colab** - [🔗 Run in Browser **(Recommended ✅)**](https://colab.research.google.com/github/hrnrxb/Data_Visualization/blob/main/Introduction_to_Data_Visualization_with_Python.ipynb)

**📁 Jupyter Notebook** - [🔗 Run locally](https://github.com/hrnrxb/Data_Visualization/blob/main/Introduction_to_Data_Visualization_with_Python.ipynb)

**🐍 Python Applications** - [🔗 Standalone Python Scripts (For local environments experiencing Jupyter kernel memory issues)](https://github.com/hrnrxb/Data_Visualization/tree/main/python%20apps%20to%20run%20externally)

**📄 PDF Version** - [🔗 Download Notebook as PDF](https://github.com/hrnrxb/Data_Visualization/blob/main/Introduction_to_Data_Visualization_with_Python.pdf)

**📃 HTML Version** - [🔗 Download Notebook as HTML](https://github.com/hrnrxb/Data_Visualization/blob/main/Introduction_to_Data_Visualization_with_Python.html)

---

## 📚 Additional Resources

**📝 Medium Article** - [🔗 Medium(not deep😅🤪) Dive Tutorial](https://hrnrxb.medium.com/5a3fe4a3852a?source=friends_link&sk=e2323739deb7e0affaddbe341bf187a3)

**🎥 Workshop Recording** - [🔗 Watch Full Session](#) *(Coming Soon)*

---

## 🛠️ Workshop Coverage

- **Matplotlib Fundamentals** - Master the foundation of Python visualization
- **Seaborn Statistical Plots** - Create publication-ready statistical graphics  
- **Plotly Interactive Visualizations** - Build dynamic, web-ready charts
- **Data Preprocessing** - Clean and prepare data for visualization
- **Best Practices** - Industry standards for effective data storytelling
- **Real-world Projects** - Hands-on exercises with actual datasets

---

## 🎯 Learning Outcomes

By completing this workshop, participants will be able to:
- Create professional-grade visualizations using Python
- Choose appropriate chart types for different data scenarios
- Build interactive dashboards and web-ready plots
- Apply data visualization best practices and design principles
- Transform complex datasets into clear, actionable insights

---

## 👥 Workshop Instructors

**[Hamid Namjoo](https://hamidnamjoo.com)** - Instructor

**[Shiraz Linux Users Group](https://shirazlug.ir/)** - Workshop Host & Technical Support

---

## 📜 License

This educational content is provided under [CC0 1.0 Universal License](https://github.com/hrnrxb/Data_Visualization/blob/main/LICENSE).

---

## 🤝 Contributing

Found an issue or want to contribute? We welcome:
- Bug reports and fixes
- Additional examples and datasets  
- Translation to other languages
- Improvements to documentation

---

## 📬 Contact & Support

- **Questions?** Open an issue in this repository
- **Professional Inquiries:** [Contact Hamid Namjoo](https://hamidnamjoo.com)
- **Community:** Join [Shiraz LUG](https://shirazlug.ir/) for ongoing supports and events✌️

---

⭐ **Drop star to this repository** if you found it helpful! Share it with fellow data enthusiasts😁🙌🌸
